...
function App() {
  ...
  // New refs to hold latest state values for worker's onmessage callback
  const timerStartTimeRef = useRef(timerStartTime);
  const durationAtTimerStartRef = useRef(durationAtTimerStart);
  const isIntervalRunningRef = useRef(isIntervalRunning);
  const totalGlobalElapsedTimeRef = useRef(totalGlobalElapsedTime);

  // Effects to keep refs updated with latest state values
  useEffect(() => {
    timerStartTimeRef.current = timerStartTime;
  }, [timerStartTime]);

  useEffect(() => {
    durationAtTimerStartRef.current = durationAtTimerStart;
  }, [durationAtTimerStart]);

  useEffect(() => {
    isIntervalRunningRef.current = isIntervalRunning;
  }, [isIntervalRunning]);

  useEffect(() => {
    totalGlobalElapsedTimeRef.current = totalGlobalElapsedTime;
    ...
    self.onmessage = function(e) {
      if (e.data.command === 'start') {
        ...
        timerInterval = setInterval(() => {
          self.postMessage({ type: 'tick', timestamp: Date.now() }); // Send timestamp
        }, delay);
  ...
  useEffect(() => {
    ...
    timerWorkerRef.current.onmessage = function(e) {
      if (e.data.type === 'tick') {
        const currentTime = e.data.timestamp;
        // Use refs to get latest state values
        const latestTimerStartTime = timerStartTimeRef.current;
        const latestDurationAtTimerStart = durationAtTimerStartRef.current;
        const latestIsIntervalRunning = isIntervalRunningRef.current;
        const latestTotalGlobalElapsedTime = totalGlobalElapsedTimeRef.current;

        const elapsedSinceStart = Math.floor((currentTime - latestTimerStartTime) / 1000);

        if (latestIsIntervalRunning) {
          const newIntervalTimeLeft = Math.max(0, latestDurationAtTimerStart - elapsedSinceStart);
          setIntervalTimeLeft(newIntervalTimeLeft);
        } else {
          const newTimeLeft = Math.max(0, latestDurationAtTimerStart - elapsedSinceStart);
          setTimeLeft(newTimeLeft);
        }
        setTotalGlobalElapsedTime(latestTotalGlobalElapsedTime + 1); // Update with latest value + 1
      }
    };
  ...
  }, []); // Removido timerWorkerCode e outras dependências, agora usa refs
